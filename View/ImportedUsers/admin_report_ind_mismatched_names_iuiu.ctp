<div class="ImportedUsers form">
    ...
    <h2><?php echo __('Report of Mismatched Imported User Names for Individual Members'); ?></h2>

    <p>This report shows the imported user records where the name doesn't match their other imported user records for individual members.</p>
    ...
    <?php for ($c1 = 0; $c1 < $line_count; $c1++) { ?>
        ...
        <td class="actions">
            <?php
                echo $this->Form->postLink(__('Delete'), array('action' => 'delete_ind_mismatched_iuiu', $mismatchedLines[$c1]['ImportedUser']['bca_no']),
...
<div class="actions">
    ...
    <ul>
        <li><?php if ($line_count <> 0) {echo $this->Html->link(__('Send As Email'), array('action'=>'email_ind_mismatched_names_iuiu'));} ?> </li>
        <li><?php if ($line_count <> 0) {echo $this->Form->postLink(__('Tidy'), array('action' => 'tidy_ind_mismatched_names_iuiu'), null,
