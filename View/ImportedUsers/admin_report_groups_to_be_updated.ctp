<div class="userAudits index">
    ...
    <h2><?php echo __('Report Of Group Members To Be Updated'); ?></h2>
    <p>This reports shows the changes to be applied to the group members in the master file.</p>
    <h3><?php
        $line_count = count($updatedLines);
        if ($line_count == 0) {
            echo "There are no group members to be updated.";
        } elseif ($line_count == 1) {
            echo "There is 1 group member to be updated.";
        } elseif ($line_count >= 1000) {
            echo "There are more than 1000 group members to be updated.";
        } else {
            echo "There are $line_count group members to be updated.";
        }
    ?></h3>
    ...
    <tr class='bold'>
        ...
        <td><?php if($updatedLine['ImportedUser']['organisation'] != $updatedLine['User']['organisation'])
            if(!empty($updatedLine['ImportedUser']['organisation'])) {
                echo h($updatedLine['ImportedUser']['organisation']);
